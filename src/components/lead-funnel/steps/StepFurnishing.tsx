...
export default function StepFurnishing() {
  ...
  return (
    <div className="flex flex-col items-center justify-center text-center max-w-2xl mx-auto px-6 gap-10">
      <div>
        <p className="text-[10px] uppercase tracking-[0.3em] text-white/50 font-bold mb-4">FURNISHING</p>
        ...
        {FURNISHING_OPTIONS.map((type) => {
          ...
          return (
            <button
              key={type}
              onClick={() => toggle(type)}
              className={`border px-6 py-4 cursor-pointer transition-all text-center ${
                selected
